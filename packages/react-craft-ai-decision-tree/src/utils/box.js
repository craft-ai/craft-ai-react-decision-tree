...
export default class Box extends Record({
  ...
  constructor(box = {
    ...
    super({
      ...
      delta: new Vector(
        (box.delta && box.delta.x) || (!_.isUndefined(box.right) && !_.isUndefined(box.left) && (box.right - box.left)) || 0,
        (box.delta && box.delta.y) || (!_.isUndefined(box.bottom) && !_.isUndefined(box.top) && (box.bottom - box.top)) || 0
      )
    });
  }
  applyMargin(margin) {
